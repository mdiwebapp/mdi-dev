...
import { memosData, memosColumns } from '../../../../data/memos-data';
...
export class MemosComponent implements OnInit {
  state: State = {
    group: [{ field: 'memos' }, { field: 'type' }],
  };
  memosForm: FormGroup;
  memosdata: GridDataResult = process(memosData, this.state);
  selections: any = [];
  multiple: boolean = false;
  safetyColumns: any = memosColumns;
  ...
  constructor(
    ...
    if (localStorage.getItem('isAdmin') == 'true') {
      ...
      let acc = this.menuService.checkUserViewRights('Memos');
      if (acc) {
      ...
      this.menuService.checkUserBySubmoduleRights('Memos');
  ...
  onInitRequestTicketForm() {
    this.memosForm = this.formBuilder.group({
      extension: [
        '',
  ...
  loadNetworkFiles() {
    this.treeViewService.getTreeViewList(82, 'Memos').subscribe(
      (res) => {
        res.forEach((element) => {
          if (element.name == null) {
            element.name = 'Memos';
            this.expandedNames = ['Memos'];
          }
  ...
  AddExtension() {
    if (this.memosForm.invalid) {
      this.memosForm.markAllAsTouched();
      return false;
    }
    this.visible = true;
    ...
    extension.extension = this.memosForm.value.extension;
    extension.description = this.memosForm.value.description;
    this.treeViewService.AddExtension(extension).subscribe((res) => {
      if (res['status'] == 200) {
        this.utility.toast.success(res['message']);
        this.memosForm.reset();
