...
export class CompanyFormsComponent implements OnInit {
  ...
  constructor(
    ...
    if (localStorage.getItem('isAdmin') == 'true') {
      ...
      let acc = this.menuService.checkUserViewRights('Company Forms');
      if (acc) {
  ...
  loadNetworkFiles() {
    this.treeViewService.getTreeViewList(26, 'FormsPath').subscribe(
      (res) => {
        res.forEach((element) => {
          if (element.name == null) {
            element.name = 'Company Forms';
            this.expandedNames = ['Company Forms'];
          }
