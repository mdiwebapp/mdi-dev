...
import { orgchartsColumns, orgchartsData } from '../../../../data/admin-data';
...
@Component({
  selector: 'app-orgCharts',
  templateUrl: './orgCharts.component.html',
  styleUrls: ['./orgCharts.component.scss'],
})
export class OrgChartsComponent implements OnInit {
  state: State = {
    group: [{ field: 'orgChart' }],
  };
  orgChartForm: FormGroup;
  requestsTickets: any = [];
  safety: GridDataResult = process(orgchartsData, this.state);
  selections: any = [];
  multiple: boolean = false;
  safetyColumns: any = orgchartsColumns;
  ...
  constructor(
    ...
    if (localStorage.getItem('isAdmin') == 'true') {
      ...
      let acc = this.menuService.checkUserViewRights('ORG Charts');
      if (acc) {
      ...
      this.menuService.checkUserBySubmoduleRights('ORG Charts');
  ...
  onInitRequestTicketForm() {
    this.orgChartForm = this.formBuilder.group({
      extension: [
        '',
  ...
  loadNetworkFiles() {
    this.treeViewService.getTreeViewList(58, 'OrgChartPath').subscribe(
      (res) => {
        res.forEach((element) => {
          if (element.name == null) {
            element.name = 'Org Charts';
            this.expandedNames = ['Org Charts'];
          }
          this.subdirectory(element);
        });
        this.data = res;
        this.lstItems =[{files:[],name:null,path:null,subDirectories:this.data}] ;
  ...
  AddExtension() {
    if (this.orgChartForm.invalid) {
      this.orgChartForm.markAllAsTouched();
      return false;
    }
    this.visible = true;
    ...
    extension.extension = this.orgChartForm.value.extension;
    extension.description = this.orgChartForm.value.description;
    this.treeViewService.AddExtension(extension).subscribe((res) => {
      if (res['status'] == 200) {
        this.utility.toast.success(res['message']);
        this.orgChartForm.reset();
