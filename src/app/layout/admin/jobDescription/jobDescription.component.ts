...
import {
  jobDescriptionColumns,
  jobDescriptionData,
} from '../../../../../src/data/admin-data';
...
export class JobDescriptionComponent implements OnInit {
  state: State = {
    group: [{ field: 'jobDescriptions' }, { field: 'type' }],
  };
  jobForm: FormGroup;
  requestsTickets: any = [];
  safety: GridDataResult = process(jobDescriptionData, this.state);
  selections: any = [];
  multiple: boolean = false;
  safetyColumns: any = jobDescriptionColumns;
  ...
  constructor(
    ...
    if (localStorage.getItem('isAdmin') == 'true') {
      ...
      let acc = this.menuService.checkUserViewRights('Job Description');
      if (acc) {
      ...
      this.menuService.checkUserBySubmoduleRights('Job Description');
  ...
  loadNetworkFiles() {
    this.treeViewService.getTreeViewList(62, 'JobDescriptionPath').subscribe(
      (res) => {
        res.forEach((element) => {
          if (element.name == null) {
            element.name = 'Job Description';
            this.expandedNames = ['Job Description'];
          }
  ...
  downloadjobDescriptionfile(data: any) {
    this.visible = false;
    this.visible = true;
  ...
  AddExtension() {
    ...
    extension.extension = this.jobForm.value.extension;
    extension.description = this.jobForm.value.description;
