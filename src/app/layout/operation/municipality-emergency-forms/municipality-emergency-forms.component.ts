...
@Component({
  selector: 'app-municipality-emergency-forms',
  templateUrl: './municipality-emergency-forms.component.html',
  styleUrls: ['./municipality-emergency-forms.component.scss'],
})
export class MunicipalityEmergencyFormsComponent implements OnInit {
  ...
  constructor(
    ...
    if (localStorage.getItem('isAdmin') == 'true') {
      ...
      let acc = this.menuService.checkUserViewRights(
        'Municipality Emergency Forms'
      );
      if (acc) {
  ...
  loadNetworkFiles() {
    this.treeViewService
      .getTreeViewList(153, 'Municipality Emergency Forms')
      .subscribe(
        (res) => {
          res.forEach((element) => {
            if (element.name == null) {
              element.name = 'Municipality Emergency Forms';
              this.expandedNames = ['Municipality Emergency Forms'];
            }
  ...
  downloadCompanyformfile(data: any) {
    this.visible = false;
    this.visible = true;
  ...
  AddExtension() {
    ...
    extension.extension = this.companyForm.value.extension;
    extension.description = this.companyForm.value.description;
