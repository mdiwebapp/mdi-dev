...
import { photosColumns, photosData } from '../../../../data/sales-data';
...
export class PhotosComponent implements OnInit {
  state: State = {
    group: [{ field: 'photos' }, { field: 'type' }],
  };
  photosForm: FormGroup;
  requestsTickets: any = [];
  safety: GridDataResult = process(photosData, this.state);
  selections: any = [];
  multiple: boolean = false;
  safetyColumns: any = photosColumns;
  ...
  constructor(
    ...
    if (localStorage.getItem('isAdmin') == 'true') {
      ...
      let acc = this.menuService.checkUserViewRights('Photos');
      if (acc) {
  ...
  loadNetworkFiles() {
    this.treeViewService.getTreeViewList(135, 'JobPhotosPath').subscribe(
      (res) => {
        res.forEach((element) => {
          if (element.name == null) {
            element.name = 'Photos';
            this.expandedNames = ['Photos'];
          }
  ...
  AddExtension() {
    if (this.photosForm.invalid) {
      this.photosForm.markAllAsTouched();
      return false;
    }
    this.visible = true;
    ...
    extension.extension = this.photosForm.value.extension;
    extension.description = this.photosForm.value.description;
