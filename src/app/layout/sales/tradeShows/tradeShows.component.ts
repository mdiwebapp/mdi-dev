...
import { TradeshowColumns, TradeShowData } from '../../../../data/sales-data';
...
@Component({
  selector: 'app-tradeShows',
  templateUrl: './tradeShows.component.html',
  styleUrls: ['./tradeShows.component.scss'],
})
export class TradeShowComponent implements OnInit {
  state: State = {
    group: [{ field: 'tradeshow' }, { field: 'type' }],
  };
  tradeShowForm: FormGroup;
  requestsTickets: any = [];
  safety: GridDataResult = process(TradeShowData, this.state);
  selections: any = [];
  multiple: boolean = false;
  safetyColumns: any = TradeshowColumns;
  ...
  constructor(
    ...
    if (localStorage.getItem('isAdmin') == 'true') {
      ...
      let acc = this.menuService.checkUserViewRights('Trade Shows');
        if (acc) {
  ...
  onInitRequestTicketForm() {
    this.tradeShowForm = this.formBuilder.group({
      extension: [
        '',
  ...
  loadNetworkFiles() {
    this.treeViewService.getTreeViewList(134, 'TradeshowsPath').subscribe(
      (res) => {
        res.forEach((element) => {
          if (element.name == null) {
            element.name = 'TradeShow';
            this.expandedNames = ['TradeShow'];
          }
  ...
  downloadTradeShowfile(data: any) {
    this.visible = false;
    this.visible = true;
  ...
  AddExtension() {
    if (this.tradeShowForm.invalid) {
      this.tradeShowForm.markAllAsTouched();
      return false;
    }
    this.visible = true;
    ...
    extension.extension = this.tradeShowForm.value.extension;
    extension.description = this.tradeShowForm.value.description;
    this.treeViewService.AddExtension(extension).subscribe((res) => {
      if (res['status'] == 200) {
        this.utility.toast.success(res['message']);
        this.tradeShowForm.reset();
