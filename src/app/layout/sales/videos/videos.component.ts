...
import { videosColumns, videosData } from '../../../../data/sales-data';
...
export class VideosComponent implements OnInit {
  state: State = {
    group: [{ field: 'videos' }, { field: 'type' }],
  };
  videosForm: FormGroup;
  requestsTickets: any = [];
  safety: GridDataResult = process(videosData, this.state);
  selections: any = [];
  multiple: boolean = false;
  safetyColumns: any = videosColumns;
  ...
  constructor(
    ...
    if (localStorage.getItem('isAdmin') == 'true') {
      ...
      let acc = this.menuService.checkUserViewRights('Videos');
      if (acc) {
  ...
  loadNetworkFiles() {
    this.treeViewService.getTreeViewList(2, 'VideoPath').subscribe(
      (res) => {
        res.forEach((element) => {
          if (element.name == null) {
            element.name = 'Videos';
            this.expandedNames = ['Videos'];
          }
  ...
  AddExtension() {
    if (this.videosForm.invalid) {
      this.videosForm.markAllAsTouched();
      return false;
    }
    this.visible = true;
    ...
    extension.extension = this.videosForm.value.extension;
    extension.description = this.videosForm.value.description;
