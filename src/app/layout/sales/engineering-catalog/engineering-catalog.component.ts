...
@Component({
  selector: 'app-engineering-catalog',
  templateUrl: './engineering-catalog.component.html',
  styleUrls: ['./engineering-catalog.component.scss'],
})
export class EngineeringCatalogComponent implements OnInit {
  catalogs: any = [];
  state: State = {
    group: [
      { field: 'type' },
  ...
  engineeringcatalogForm: FormGroup;
  ...
  constructor(
    ...
    if (localStorage.getItem('isAdmin') == 'true') {
      ...
      let acc = this.menuService.checkUserViewRights('Engineering Catalog');
      if (acc) {
  ...
  onInitRequestTicketForm() {
    this.engineeringcatalogForm = this.formBuilder.group({
      extension: [
        '',
  ...
  sortGridChange(event) {}

  loadNetworkFiles() {
    this.treeViewService.getTreeViewList(132, 'Presentations').subscribe(
      (res) => {
        res.forEach((element) => {
          if (element.name == null) {
            element.name = 'Engineering Catalog';
            this.expandedNames = ['Engineeering Catalog'];
          }
  ...
  downloadEngineeringCatalogfile(data: any) {
    this.visible = false;
    this.visible = true;
  ...
  AddExtension() {
    if (this.engineeringcatalogForm.invalid) {
      this.engineeringcatalogForm.markAllAsTouched();
      return false;
    }
    this.visible = true;
    ...
    extension.extension = this.engineeringcatalogForm.value.extension;
    extension.description = this.engineeringcatalogForm.value.description;
    this.treeViewService.AddExtension(extension).subscribe((res) => {
      if (res['status'] == 200) {
        this.utility.toast.success(res['message']);
        this.engineeringcatalogForm.reset();
