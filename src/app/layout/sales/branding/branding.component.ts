...
import { BrandingColumns, BrandingData } from '../../../../data/sales-data';
...
export class BrandingComponent implements OnInit {
  state: State = {
    group: [{ field: 'branding' }, { field: 'type' }, { field: 'group' }],
  };
  brandingForm: FormGroup;
  requestsTickets: any = [];
  safety: GridDataResult = process(BrandingData, this.state);
  selections: any = [];
  multiple: boolean = false;
  safetyColumns: any = BrandingColumns;
  ...
  constructor(
    ...
    if (localStorage.getItem('isAdmin') == 'true') {
      ...
      let acc = this.menuService.checkUserViewRights('Branding');
      if (acc) {
  ...
  loadNetworkFiles() {
    this.treeViewService.getTreeViewList(131, 'BrandingGuide').subscribe(
      (res) => {
        res.forEach((element) => {
          if (element.name == null) {
            element.name = 'Branding';
            this.expandedNames = ['Branding'];
          }
  ...
  AddExtension() {
    ...
    extension.extension = this.brandingForm.value.extension;
    extension.description = this.brandingForm.value.description;
