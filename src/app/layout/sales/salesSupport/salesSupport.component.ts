...
@Component({
  selector: 'app-salesSupport',
  templateUrl: './salesSupport.component.html',
  styleUrls: ['./salesSupport.component.scss'],
})
export class SalesSupportComponent implements OnInit {
  state: State = {
    group: [{ field: 'support' }, { field: 'type' }],
  };
  salesSuportForm: FormGroup;
  requestsTickets: any = [];
  safety: GridDataResult = process(salesSupportData, this.state);
  selections: any = [];
  multiple: boolean = false;
  safetyColumns: any = salesSupportColumns;
  ...
  constructor(
    ...
    if (localStorage.getItem('isAdmin') == 'true') {
      ...
      let acc = this.menuService.checkUserViewRights('Sales Support');
      if (acc) {
  ...
  onInitRequestTicketForm() {
    this.salesSuportForm = this.formBuilder.group({
      extension: [
        '',
  ...
  loadNetworkFiles() {
    this.treeViewService.getTreeViewList(133, 'MarketingLiterature').subscribe(
      (res) => {
        res.forEach((element) => {
          if (element.name == null) {
            element.name = 'Sales Support';
            this.expandedNames = ['Sales Support'];
          }
  ...
  // public children = (dataitem: any): Observable<any[]> => of(this.getItems(dataitem));
  ...
  downloadSalesSupportfile(data: any) {
    this.visible = false;
    this.visible = true;
  ...
  AddExtension() {
    if (this.salesSuportForm.invalid) {
      this.salesSuportForm.markAllAsTouched();
      return false;
    }
    this.visible = true;
    ...
    extension.extension = this.salesSuportForm.value.extension;
    extension.description = this.salesSuportForm.value.description;
    this.treeViewService.AddExtension(extension).subscribe((res) => {
      if (res['status'] == 200) {
        this.utility.toast.success(res['message']);
        this.salesSuportForm.reset();
