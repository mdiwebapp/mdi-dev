...
export class TimeClockComponent implements OnInit {
  ...
  onValueChange(value) {
    if (this.isJobDisabled && !this.isEmployeeDisabled) {
      this.inputValue += value;
      this.timeClock.controls['EmployeeNo'].setValue(this.inputValue);
    }
    if (this.isEmployeeDisabled && !this.isJobDisabled) {
      this.jobValue += value;
  ...
  GetEmployeeHourDetails() {
    ...
    if (!isNaN(EmployeeNo)) {
      ...
      this.service.GetEmployeeHourDetail(EmployeeNo).subscribe(
        (res) => {
          ...
          if (res.dayHourList != null) {
            this.Daysdata = res.dayHourList.map((x) => {
              return {
                objtimeIn: this.datepipe.transform(
                  x.timeIn,
                  'MM/dd/yyyy hh:mm:ss a'
                ),
                objtimeOut: this.datepipe.transform(
                  x.timeOut,
                  'MM/dd/yyyy hh:mm:ss a'
                ),
                ...x,
          ...
          if (!res.punchedIn) {
            ...
            if (punchedOuttime != null && punchedOuttime != '') {
              ...
              var punchinTime = new Date(
                objpunchOuttime.setMinutes(objpunchOuttime.getMinutes() + 30)
              );
              ...
              if (currenttime > punchinTime) {
                ...
                this.timediff = Math.ceil(
                  (punchinTime.getTime() - currenttime.getTime()) / 60000
                );
          ...
          if (
            res.punchedIn &&
            this.timeClock.value.JobName != null &&
            this.timeClock.value.JobName != ''
          ) {
  ...
  loadJobDetail() {
    ...
    this.service.GetJobDetail(JobNo).subscribe(
      (res) => {
        if (res.length > 0) {
  ...
  GetPTODetail() {
    ...
    if (EmployeeNo != '' && EmployeeNo != null) {
      this.visible = false;
      this.visible = true;
      this.service.GetPTODetailEmployeeNo(EmployeeNo).subscribe(
        (res) => {
          if (res.ptoList != null) {
            if (res.ptoList.length > 0) {
              this.PTOData = res.ptoList.map((x) => {
                return { objworkDate: new Date(x.workDate), ...x };
              });
  ...
  OnPunchIn() {
    ...
    var request = {
      ...
      employeeNumber: this.selectedEmployeeNo,
      jobNumber: this.selectedJobNo,
      previousJobNumber: previousjobNumber,
    };
    this.service.PunchInEmployee(request).subscribe(
      (res) => {
        if (res['status'] == 200) {
          this.isPunchInVisible = false;
          this.isPunchOUTVisible = true;
  ...
  public closePunchOutdialog(data) {
    ...
    if (data == 'yes') {
      ...
      this.service.PunchOutEmployee(request).subscribe(
        (res) => {
          if (res['status'] == 200) {
            this.isdialogPunchOutVisible = false;
            this.isPunchInVisible = false;
  ...
  loadHoursData() {
    if (this.selectedEmployeeNo > 0) {
      this.visible = false;
      this.visible = true;
      this.service.GetEmployeeHourDetail(this.selectedEmployeeNo).subscribe(
        (res) => {
          if (res.dayHourList != null) {
            this.Daysdata = res.dayHourList.map((x) => {
              return {
